  ...
  checkInvalidIds(steps) {
    for (const key in steps) {
      ...
      if (typeof triggerId !== 'function') {
        if (step.options) {
          const stepOptions = (typeof step.options === 'function') ? step.options(step, steps) : step.options;
          if (typeof stepOptions !== 'function') {
            const triggers = stepOptions.filter(option => typeof option.trigger !== 'function');
